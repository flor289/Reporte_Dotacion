...
if archivo:
    try:
        ...
        if not df_salidas.empty:
            ...
            df_salidas['Mes_Nombre'] = df_salidas['Mes_Num'].map(MESES_ES)
            df_salidas['Mes_Display'] = df_salidas['Mes_Nombre'] + " " + df_salidas['Anio'].astype(str)
            # Para ordenar cronológicamente
            df_salidas['Mes_Sort'] = df_salidas['Fecha_Real'].dt.strftime('%Y-%m')
            
            df_grafico = df_salidas.groupby(['Mes_Sort', 'Mes_Display', 'Tipo']).size().reset_index(name='Cantidad')
            df_grafico = df_grafico.sort_values('Mes_Sort')
            ...
            if st.button("📄 Generar Reporte PDF"):
                ...
                pdf.image(io.BytesIO(img_bytes), x=10, y=None, w=220)
                pdf.ln(5)
                ...
                st.download_button("Descargar PDF", data=pdf.output(dest='S').encode('latin-1', 'replace'), 
                                   file_name=f"Reporte_RRHH_{f_fin}.pdf", mime="application/pdf")
